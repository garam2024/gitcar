...
class InspectAdapter_1st:
    ...
    def get_inspect_list(self, request):
        ...
        sql_query = sqlMethod()
        #로그인 사용자 등록 회사 검색
        #자신이 반려햇던 작업이보이지 않게 추가(경진)[10.27]
        #작업자분들 요청으로 검수 리미트 해제(경진)[11.09]
        select_group_id = sql_query.select_workList(table_name='django_app_profile', column_list=['group_id'], data_dic={"account_id" : str(request.user)}, option=None)
        ...
        option = "and (inspect_id1 is null or inspect_id1 = '' ) order by work_id"
        try:
            inspect_list = sql_query.select_workList(table_name='django_app_worklist', data_dic={
                'work_status': dbinfo.status['status_1cha_inspect_deagi'], "group_id": select_group_id[0]['group_id']}, status_list=None, column_list=column_list,
                                                     option=option)
        except:
            raise
